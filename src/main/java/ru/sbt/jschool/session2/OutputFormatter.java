    ...
    public void output(String[] names, Object[][] data) {
        ...
        if (data.length == 0) {
            for (int i = 0; i < width; i++) {
                columns[i] = new ColumnFormatter(ColumnType.STRING);
                columns[i].addTitle(names[i]);
            }
        } else {
            for (int i = 0; i < width; i++) {
                if (data[0][i] instanceof Float || data[0][i] instanceof Double) {
                    columns[i] = new ColumnFormatter(ColumnType.MONEY);
                } else if (data[0][i] instanceof Integer) {
                    columns[i] = new ColumnFormatter(ColumnType.NUMBER);
                } else if (data[0][i] instanceof Date) {
                    columns[i] = new ColumnFormatter(ColumnType.DATE);
                } else {
                    columns[i] = new ColumnFormatter(ColumnType.STRING);
                }
                columns[i].addTitle(names[i]);
